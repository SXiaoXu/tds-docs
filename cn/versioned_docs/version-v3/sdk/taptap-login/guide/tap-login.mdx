...
单纯接入「TapTap 登录」，需要依赖 `TapLogin` 和 `TapCommon` 模块。请先 [下载](/v3/tap-download) TapSDK，并添加相关依赖。
...
1. 在 [下载页](/v3/tap-download) 找到 **TapSDK Unity** 下载地址，下载 `TapSDK-UnityPackage.zip` 
...
3. 从 [LeanCloud-SDK 下载页](https://github.com/leancloud/csharp-sdk/releases) 中下载并解压 LeanCloud-SDK-Storage-Unity.zip，然后将 Plugins 文件夹拖拽至 Unity 即可。
...
1. [下载 TapSDK Android](/v3/tap-download)，解压后选择需要用到的 SDK 包导入到项目 `project/app/libs` 目录下。
...
2. 下载 [TapSDK iOS](/v3/tap-download)，解压后选择需要导入的资源文件，直接拖拽到项目目录即可。
...
* 下载 [TapSDK UE4](/v3/tap-download)，TapSDK-UE4-xxx.zip 解压后将 `TapLogin`、`TapCommon` 文件夹 Copy 到项目的 `Plugins` 目录中
* 重启 Unreal Editor
