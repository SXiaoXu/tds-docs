...
请先参考 [TapTap 登录](/v3/sdk/taptap-login/guide/start/#sdk-获取)完成 SDK 获取，然后在此基础上可以通过 [下载](/v3/tap-download) 获得 TapSDK，添加 `TapMoment` 模块：
...
1. 请先 [下载](/v3/tap-download) TapSDK，并添加相关依赖。内嵌动态功能依赖 `TapLogin`、`TapCommon` 和 `TapMoment` 模块。
